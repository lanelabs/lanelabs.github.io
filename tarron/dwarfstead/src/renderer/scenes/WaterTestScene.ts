/**
 * WaterTestScene — Fluid Test Lab for water + gas chambers.
 *
 * Auto-advances water and gas simulations at 200ms/tick.
...
import { drawGasDebugTiles } from '../draw/gasStreamDebug';
...
import type { FluidLabMode, DrawAction } from './fluidTestHelpers';
import { MODE_CYCLE, getModeLabelParts, executeFluidAction, getDragAction, executeDragAction } from './fluidTestHelpers';
...
const SAVE_VERSION = 6; // gas reservoir layout change
const SAVE_INTERVAL_MS = 1000;
const MOVE_COOLDOWN = 60;

interface FluidLabSave {
  version?: number;
  game: SaveData;
  water: WaterSaveData;
  gas?: GasSaveData;
...
export class WaterTestScene extends Phaser.Scene {
  ...
  private gasGraphics!: Phaser.GameObjects.Graphics;
  ...
  private mode: FluidLabMode = 'water';
  ...
  constructor() { super({ key: 'WaterTestScene' }); }
  ...
  create() {
    ...
    if (saved) {
      ...
      this.waterSystem = new WaterPathSystem({
        width: testWorld.terrain.width, height: testWorld.terrain.height,
        blocks: this.simGame.terrain.blocks, pipes: testWorld.pipes,
        pumps: testWorld.pumps, initialWaterVolume: testWorld.initialWaterVolume,
      });
      WaterPathSystem.restoreState(this.waterSystem, saved.water);
      this.gasSystem = new GasPathSystem({
        width: testWorld.terrain.width, height: testWorld.terrain.height,
        blocks: this.simGame.terrain.blocks, pipes: testWorld.pipes,
        pumps: testWorld.pumps, initialGasVolume: testWorld.initialGasVolume,
      });
      if (saved.gas) GasPathSystem.restoreState(this.gasSystem, saved.gas);
    } else {
      this.simGame = new Game({
        seed: 1, worldWidth: testWorld.terrain.width,
        worldHeight: testWorld.terrain.height, startingDwarves: 0,
      ...
      this.waterSystem = new WaterPathSystem({
        width: testWorld.terrain.width, height: testWorld.terrain.height,
        blocks: testWorld.terrain.blocks, pipes: testWorld.pipes,
        pumps: testWorld.pumps, initialWaterVolume: testWorld.initialWaterVolume,
      });
      this.gasSystem = new GasPathSystem({
        width: testWorld.terrain.width, height: testWorld.terrain.height,
    ...
    this.entityGraphics = this.add.graphics().setDepth(6);
    this.waterGraphics = this.add.graphics().setDepth(3);
    this.gasGraphics = this.add.graphics().setDepth(4);
    this.pipeGraphics = this.add.graphics().setDepth(5);
    ...
    this.add.text(w / 2, 10, 'FLUID TEST LAB', {
    ...
    zoomInBtn.on('pointerdown', () => { this.setZoom(this.zoomIndex + 1); this.updateCamera(); this.redraw(); });
    ...
    const zoomOutBtn = this.add.text(w - 10, 30, '[-]', btnStyle)
      .setOrigin(1, 0).setDepth(100).setInteractive({ useHandCursor: true });
    zoomOutBtn.on('pointerdown', () => { this.setZoom(this.zoomIndex - 1); this.updateCamera(); this.redraw(); });
    zoomOutBtn.on('pointerover', () => zoomOutBtn.setColor('#ffffff'));
    zoomOutBtn.on('pointerout', () => zoomOutBtn.setColor('#e8c170'));
    const resetBtn = this.add.text(w - 10, 52, '[Reset Fluids]', btnStyle)
      .setOrigin(1, 0).setDepth(100).setInteractive({ useHandCursor: true });
    resetBtn.on('pointerdown', () => this.resetFluids());
    resetBtn.on('pointerover', () => resetBtn.setColor('#ff6666'));
    resetBtn.on('pointerout', () => resetBtn.setColor('#e8c170'));

    const kb = this.input.keyboard!;
    const K = Phaser.Input.Keyboard.KeyCodes;
    this.keyW = kb.addKey(K.W); this.keyA = kb.addKey(K.A);
    this.keyS = kb.addKey(K.S); this.keyD = kb.addKey(K.D);
    this.keyEsc = kb.addKey(K.ESC); this.keyPlus = kb.addKey(K.PLUS);
    this.keyMinus = kb.addKey(K.MINUS); this.keySpace = kb.addKey(K.SPACE);
  ...
  update(_time: number, delta: number) {
    ...
    if (this.inputCooldown <= 0) {
      ...
      if (Phaser.Input.Keyboard.JustDown(this.keyTab)) {
        this.mode = MODE_CYCLE[(MODE_CYCLE.indexOf(this.mode) + 1) % MODE_CYCLE.length];
        this.updateModeText();
      }
      let moved = false;
      ...
      const isDragMode = this.mode === 'pipe' || this.mode === 'stone';
      if (isDragMode) {
        if (Phaser.Input.Keyboard.JustDown(this.keySpace)) {
          this.startDrag(); moved = true;
        } else if (this.keySpace.isDown && this.drawAction && moved) {
          this.continueDrag();
        }
      } else {
        if (Phaser.Input.Keyboard.JustDown(this.keySpace)) { this.executeAction(); moved = true; }
      }
      if (this.keyPlus.isDown) { this.setZoom(this.zoomIndex + 1); moved = true; }
      if (this.keyMinus.isDown) { this.setZoom(this.zoomIndex - 1); moved = true; }
      if (moved) { this.inputCooldown = MOVE_COOLDOWN; this.updateCamera(); ticked = true; }
    }
    if (!this.keySpace.isDown) this.drawAction = null;
  ...
  private loadSave(): FluidLabSave | null {
    try {
      const raw = localStorage.getItem(SAVE_KEY);
      if (!raw) return null;
      const data = JSON.parse(raw) as FluidLabSave;
      if (data.version !== SAVE_VERSION) { localStorage.removeItem(SAVE_KEY); return null; }
      return data;
    } catch { return null; }
  }
  ...
  private executeAction(): void {
    ...
    if (!dwarf) return;
    const pos = dwarf.get<PositionComponent>('position')!;
    executeFluidAction(this.mode, pos.x, pos.y, this.waterSystem, this.gasSystem, this.simGame.terrain.blocks);
    this.saveState();
  }
  ...
  private startDrag(): void {
    ...
    if (!dwarf) return;
    const pos = dwarf.get<PositionComponent>('position')!;
    this.drawAction = getDragAction(this.mode, pos.x, pos.y, this.waterSystem, this.simGame.terrain.blocks);
    if (this.drawAction) {
      executeDragAction(this.mode, this.drawAction, pos.x, pos.y, this.waterSystem, this.gasSystem, this.simGame.terrain.blocks);
      this.saveState();
    }
  }
  ...
  private updateModeText(): void {
    this.modeText.setText(getModeLabelParts(this.mode));
  }

  private redraw(): void {
    const blocks = this.simGame.terrain.blocks;
    drawTerrain(this.terrainGraphics, this.simGame, this.ts, this.tilesX, this.tilesY, this.camX, this.camY);
    drawEntities(this.entityGraphics, this.simGame, this.ts, this.scale.width, this.scale.height, this.camX, this.camY, false);
    const waterArms = drawWater(this.waterGraphics, this.waterSystem.state, this.ts, this.tilesX, this.tilesY, this.camX, this.camY, blocks);
    const gasArms = drawGas(this.gasGraphics, this.gasSystem.state, this.ts, this.tilesX, this.tilesY, this.camX, this.camY, blocks);
    drawAllPipes(this.pipeGraphics, this.waterSystem.state.pipes, this.waterSystem.state.pumps, blocks, waterArms, gasArms, this.ts, this.tilesX, this.tilesY, this.camX, this.camY);
    this.debugLabels = drawWaterDebug(this, this.debugLabels, this.waterSystem.state, this.ts, this.tilesX, this.tilesY, this.camX, this.camY);
    this.streamDebugGraphics.clear();
    this.streamDebugLabels = drawGasDebugTiles(this, this.streamDebugGraphics, this.streamDebugLabels, this.ts);
  }
}
